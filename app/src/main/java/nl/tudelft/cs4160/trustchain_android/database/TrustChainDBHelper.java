...
public class TrustChainDBHelper extends SQLiteOpenHelper {
    ...
    private final String SQL_CREATE_ENTRIES_2 =
            "CREATE TABLE IF NOT EXISTS block_description(type TEXT PRIMARY KEY, value TEXT NOT NULL, sequence_number INTEGER NOT NULL);";
    ...
    public long insertInDB(MessageProto.TrustChainBlock block, int type, String value) {
        ...
        long result = db.insert(TrustChainDBContract.BlockEntry.TABLE_NAME, null, values);
        if (result != -1) {
            if (type != -1) {
                //store the postion of this block
                db = getWritableDatabase();
                values = new ContentValues();
                values.put("type", type);
                values.put("value", value);
                values.put("sequence_number", block.getSequenceNumber());
                return db.insert("block_description", null, values);
            }
    ...
    //Get the description (position and value) of all the block that you are storing
    public ArrayList<BlockDescription> getBlockDescriptionStored() {
        ArrayList<BlockDescription> blockDescriptions = new ArrayList<BlockDescription>();

        SQLiteDatabase dbReadable = getReadableDatabase();
        String[] projection = new String[]{"type", "value", "sequence_number"};
        ...
        if (cursor.getCount() > 0) {
            ...
            while (true) {
                blockDescriptions.add(new BlockDescription(cursor.getInt(cursor.getColumnIndex("type")), cursor.getString(cursor.getColumnIndex("value")), cursor.getInt(cursor.getColumnIndex("sequence_number"))));
                if (cursor.isLast()) {
                    break;
                }
                cursor.moveToNext();
    ...
    //Get the sequence position of a block give the Type ID (check the Types class), if it return null mean that we don't have a block that store that type of information
    public BlockDescription getBlockDescriptionByTypeID(int typeID) {
        ...
        String[] whereArgs = new String[]{Integer.toString(typeID)};
        ...
        if (cursor.getCount() > 0) {
            cursor.moveToFirst();
            blockDescription = new BlockDescription(cursor.getInt(cursor.getColumnIndex("type")), cursor.getString(cursor.getColumnIndex("value")), cursor.getInt(cursor.getColumnIndex("sequence_number")));
        }
        cursor.close();
